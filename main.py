...
##CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://blog_8ndh_user:KYXIo0Por2GnpySyxUAY678xblczhzM6@dpg-cepnj95a4991ihim9r3g-a.oregon-postgres.render.com/blog_8ndh'
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user.is_anonymous or current_user.id != 1:
            print("OUT NOW")
            abort(403)
            return redirect(url_for('forbidden_page', next=request.url))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        requested_user = User.query.filter_by(email=the_email).first()
        if requested_user:
            flash("You've already signed up with that email, log in instead!", 'error')
            return redirect(url_for("login"))

        not_hashed_password = form.password.data
        hashed_password = generate_password_hash(not_hashed_password, method="pbkdf2:sha256", salt_length=8)
        new_user = User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if requester_user == None:
            ...
            login_user(user=requester_user)
            return redirect(url_for("get_all_posts"))
        else:
